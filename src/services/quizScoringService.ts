...
export class QuizScoringService {
  ...
  static calculateScore(
    ...
    const score = Math.round((correctAnswers * pointsPerQuestion) * 100) / 100; // Arredonda para 2 casas decimais
    const percentage = Math.round(((correctAnswers / questions.length) * 100) * 100) / 100; // Arredonda para 2 casas decimais
  ...
  static calculateStudentStats(attempts: QuizAttempt[]): StudentQuizStats {
    ...
    return {
      ...
      averageScore: Math.round((scores.reduce((sum, s) => sum + s, 0) / scores.length) * 100) / 100, // Arredonda para 2 casas decimais
      averagePercentage: Math.round((percentages.reduce((sum, p) => sum + p, 0) / percentages.length) * 100) / 100, // Arredonda para 2 casas decimais
